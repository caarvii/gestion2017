...
IF  (@cant_chofer > 0) BEGIN
		...
		throw 60000, @error_message_patente , 1; 
END
ELSE
...
declare @cant int
set @cant = (SELECT COUNT (*) FROM GARBAGE.ChoferxAutomovil WHERE chof_auto_chof_id=@auto_chofer_id AND chof_auto_habilitado = 0);
